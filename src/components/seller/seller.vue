...
<script type="text/ecmascript-6">
  ...
  import {CONSTANT} from '@/common/js/constant'; // 滚动条JS
  ...
  export default {
    ...
    data() {
      return {
        ...
        picScroll: null,
        favorite: (() => {
          return loadFromLocal(this.seller.id, CONSTANT.LOCAL_STORAGE_KEY.FAVORITE);
        })()
      };
    },
    // 当dom渲染完毕后加载滚动控件
    ...
    methods: {
      ...
      toggleFavorite(event) {
        if (event._constructed) {
          ...
          saveToLocal(this.seller.id, CONSTANT.LOCAL_STORAGE_KEY.FAVORITE, this.favorite);
        }
      }
    },
    computed: {
  ...
  .seller
    ...
    .overview
      ...
      .favorite
        ...
        right: 18px
        top: 18px
        // 指定"width + text-align: center",使文本变更时图标的位置不改变
        width: 36px
        text-align: center
        .icon-favorite
          // 折行水平居中
